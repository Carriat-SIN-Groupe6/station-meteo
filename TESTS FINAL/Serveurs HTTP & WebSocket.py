...
<!DOCTYPE html>
<html lang='fr'>
<head><title>Station Météo</title></head>
<body>
<h1>Station Météo</h1>
<p>Température: {bme280.temperature()}°C</p>
<p>Humidité: {bme280.humidity()}%</p>
<p>Pression: {bme280.pressure()} hPa</p>
<script>
    const ws = new WebSocket('ws://' + window.location.host + ':{WS_PORT}');
    ws.onmessage = event => {
        const data = JSON.parse(event.data);
        document.body.innerHTML = `<h1>Station Météo</h1>
