...
while True:
    if wlan.status() == 3:
        ...
        res = requests.get(url='https://timeapi.io/api/time/current/coordinate?latitude=46.197376&longitude=5.223058')
        ...
        ledmatrix.off()
        res_json = res.json()
        print(f"There is {res_json["time"]}:{res_json["seconds"]}.")
